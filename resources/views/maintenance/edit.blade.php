...
<body class="bg-body">
    ...
    <div class="container-fluid">
        ...
        <form method="POST" action="{{ route('maintenance.update', $maintenance->id) }}">
            ...
            <div class="mb-3">
                <label for="proyect_id" class="form-label">Seleccione un Proyecto</label>
                <select class="form-select" id="proyect_id" name="proyect_id" required>
                    @foreach($proyects as $proyect)
                        <option value="{{ $proyect->id }}" {{ ($maintenance->proyect_id == $proyect->id) ? 'selected' : '' }}>
                            {{ $proyect->area_raiz }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="staff_id" name="staff_id" required>
                    @foreach($staffMembers as $staff)
                        <option value="{{ $staff->id }}" {{ ($maintenance->staff_id == $staff->id) ? 'selected' : '' }}>
                            {{ $staff->full_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="fecha_inicio" name="fecha_inicio" value="{{ old('fecha_inicio', $maintenance->fecha_inicio) }}" required>
            </div>

            <div class="mb-3">
                <label for="fecha_final" class="form-label">Fecha Final</label>
                <input type="date" class="form-control" id="fecha_final" name="fecha_final" value="{{ old('fecha_final', $maintenance->fecha_final) }}" required>
            </div>

            <div class="mb-3">
                <label for="estado" class="form-label">Estado</label>
                <select class="form-select" id="estado" name="estado" required>
                    <option value="abierto" {{ old('estado', $maintenance->estado) == 'abierto' ? 'selected' : '' }}>Abierto</option>
                    <option value="pausado" {{ old('estado', $maintenance->estado) == 'pausado' ? 'selected' : '' }}>Pausado</option>
                    <option value="cerrado" {{ old('estado', $maintenance->estado) == 'cerrado' ? 'selected' : '' }}>Cerrado</option>
                </select>
            </div>

            <button type="submit" class="bg-orange-gradient px-6 py-1 font-black">Guardar Cambios</button>
            <button class="bg-orange-gradient px-6 py-1 font-black"><a href="{{ route('maintenance.index') }}">Cancelar</a></button>
        </form>
    </div>
